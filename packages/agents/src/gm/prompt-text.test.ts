// A guard, not a behaviour test. `GM_PROMPT_VERSION` is only useful to
// step 7b — and to whichever event payload eventually logs it — if it
// actually changes when the prompt changes. A version someone forgets to
// bump is worse than no version at all, because it labels two different
// prompts identically in the log. Mirrors `intent/prompt-text.test.ts`.
//
// So this pins a hash of every prompt string that reaches the model. Editing
// any of them fails this test, and the only way to make it pass is to bump the
// version and re-pin the hash. The hashing lives here rather than in the
// production module so `@ai-dm/agents` keeps no crypto dependency.
import { createHash } from "node:crypto";
import { describe, expect, it } from "vitest";
import {
  GM_PROMPT_VERSION,
  GM_SYSTEM_PROMPT,
  GM_TOOL_DESCRIPTION,
  GM_TOOL_NAME,
} from "./prompt-text.js";

/** Every string this package sends to a model, in a fixed order. */
const PROMPT_SURFACE = [GM_TOOL_NAME, GM_TOOL_DESCRIPTION, GM_SYSTEM_PROMPT].join(" ");

/** Bump `GM_PROMPT_VERSION` and re-pin this together, never separately. */
const PINNED = {
  version: "gm-v1",
  sha256: "5aafed4aaf8911a4636326bb21d52f994473a9e9b27410014bba508a542701f7",
};

describe("prompt version guard", () => {
  it("fails when a prompt is edited without bumping the version", () => {
    const actual = createHash("sha256").update(PROMPT_SURFACE, "utf8").digest("hex");

    expect(actual).toBe(PINNED.sha256);
  });

  it("pins the hash against the version it was taken from", () => {
    expect(GM_PROMPT_VERSION).toBe(PINNED.version);
  });
});
