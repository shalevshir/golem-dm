...
The orchestrator: Fastify + `@fastify/websocket`. Owns the turn pipeline, the campaign lifecycle — an encounter is a bracketed span inside it, not a lifecycle of its own — and the append-only event log. The ONLY place where rules-engine, agents, and memory meet — none of those packages may import each other's runtime services directly.
