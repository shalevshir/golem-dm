...
// carry. `SessionState` is the board — combatants, grid, turn order — and a
...
//
// `sessionStorage`, not `localStorage`, for the same reason the session id
...
export const LOG_STORAGE_KEY = "ai-dm:session-log";

/**
 * `sessionId` is stored alongside the rest and checked on the way back in.
...
export function storeClientState(sessionId: string, state: ClientState): void {
 ...
 */
export function restoreClientState(sessionId: string | null): ClientState {
  const raw = sessionStorage.getItem(LOG_STORAGE_KEY);
  if (sessionId === null || raw === null) return initialClientState;
  ...
  if (parsed.data.sessionId !== sessionId) return initialClientState;
