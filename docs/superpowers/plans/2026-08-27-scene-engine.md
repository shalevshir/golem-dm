...
| 7 Docs sweep | `05e6c75` | Sweep of every "scene engine"/`AuthoredWorld`/`pairKey` hit outside `packages/memory/CLAUDE.md` (excluded per controller instruction — someone else's uncommitted edit). Postgres run (migrated onto scratch `aidm_step3`): 95 files, 1407 passed, **0 skipped**, `packages/memory` 62/62. Without `DATABASE_URL`: 95 files, 1377 passed, 30 skipped. `pnpm typecheck` and `npx eslint packages apps tools` both exit 0. |
| Whole-branch review + fix wave | `8d90364`, `a73fc28` | Two Important, four Minor, four stale doc claims. Found the fourth check that could not fail: `availableEdges`' unknown-node branch, hidden because its `if` and `return` share a line, so v8 called it covered at 100% lines while branch coverage was 95.23%. |
| Post-merge-prep review | `f9e04e9`, `79b7c51`, `7c411ca` | Three findings on the same fault line — `SceneState` had one producer, and each bug only bites when step 4 adds a second. `applyEffect` and `evaluatePredicate` now read bands through one authored-baseline lookup; `completeCurrentNode` re-checks its own preconditions, then had its idempotency restored when that check was found to run ahead of the already-completed short-circuit. |
